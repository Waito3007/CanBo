...
namespace DoAnNhom1.Controllers
{
    ...
    public class TreeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                return View(database.Tree.ToList());
            }
        }
        public ActionResult SelectRegion()
        {
            Region se_city = new Region();
            se_city.ListRe = database.Region.ToList<Region>();
        ...
        {
            var tree = database.Tree.Where(s => s.TreeID == id).FirstOrDefault();
            Session["imgPath"] = tree.ImageTree;
            ViewBag.Regions = new SelectList(database.Region.OrderBy(r => r.NameRe), "IDRe", "NameRe", tree.Region.IDRe);
            if (tree == null)
            {
